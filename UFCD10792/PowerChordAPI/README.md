...
- **SQL Server LocalDB / SQL Server Express**, caso não queiras usar a base de dados em memória
...
Extrai o projeto para uma pasta, por exemplo:

```text
C:\Users\teu_utilizador\Desktop\Prog\PowerChordAPI_sem_Docker
```
...
```powershell
cd C:\Users\teu_utilizador\Desktop\Prog\PowerChordAPI_sem_Docker\PowerChordAPI
```
...
Se tiveres Redis instalado localmente, inicia-o antes da API.
...
Se quiseres usar SQL Server LocalDB ou SQL Server Express, altera a connection string.
...
Clica no botão **Authorize** e escreve:

```text
Bearer TOKEN_AQUI
```

Depois disso, já podes testar os endpoints protegidos.
...
Podem ser usados se quiseres criar a base de dados manualmente no SQL Server.
